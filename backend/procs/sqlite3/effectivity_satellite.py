...
def generate_effectivity_satellite(data_structure):
    ...
    for object in object_list:
        ...
        for e_sat in results:
            tracked_hk = e_sat[0]
            tracked_entity = e_sat[1]
            sources = ""

            for source in list(dict.fromkeys(e_sat[2].split(','))):
            ...
            with open(os.path.join(root,"templates","effectivity_sat.txt"),"r") as f:
                command_tmp = f.read()
            f.close()
            command = command_tmp.replace('@@Schema', rdv_default_schema).replace('@@Tracked_HK', tracked_hk).replace('@@Source_Models', sources)

            filename = os.path.join(model_path , f"{tracked_entity}_es.sql")
            ...
            with open(filename, 'w') as f:
                ...
                if data_structure['console_outputs']:
                    data_structure['print2FeedbackConsole'](message= f"Created Effectivity Satellite {tracked_entity}es") 
