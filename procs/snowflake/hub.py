...
def generate_hub_list(cursor, source):
    ...
    query = f"""SELECT h.Hub_Identifier,h.Target_Hub_table_physical_name,LISTAGG(h.Business_Key_Physical_Name,',') WITHIN GROUP (ORDER BY h.Hub_Identifier,h.Target_Hub_table_physical_name desc)
                from core.hub_entities h
                inner join core.source_data src on src.Source_table_identifier = h.Source_Table_Identifier
                where 1=1
                and src.Source_System = '{source_name}'
                and src.Source_Object = '{source_object}'
                group by h.Hub_Identifier,h.Target_Hub_table_physical_name
                """
...
def generate_source_models(cursor, hub_id):
    ...
    query = f"""SELECT src.Source_Table_Physical_Name,LISTAGG(h.Source_Column_Physical_Name) WITHIN GROUP (order by Target_Column_Sort_Order),src.Static_Part_of_Record_Source_Column
                FROM core.hub_entities h
