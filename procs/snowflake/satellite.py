...
def generate_satellite_list(cursor, source):
    ...
    query = f"""SELECT DISTINCT hs.Satellite_Identifier,hs.Target_Satellite_Table_Physical_Name,hs.Hub_Primary_Key_Physical_Name,LISTAGG(hs.Target_Column_Physical_Name) WITHIN GROUP (order by Target_Column_Sort_Order asc),
                ...
                group by hs.Satellite_Identifier,hs.Target_Satellite_Table_Physical_Name,hs.Hub_Primary_Key_Physical_Name,src.Source_Table_Physical_Name,src.Load_Date_Column

UNION DISTINCT

SELECT DISTINCT ls.Satellite_Identifier,ls.Target_Satellite_Table_Physical_Name,ls.Link_primary_key_physical_name,LISTAGG(ls.Target_Column_Physical_Name) WITHIN GROUP (order by Target_Column_Sort_Order asc),
                src.Source_Table_Physical_Name,src.Load_Date_Column
                ...
                and src.Source_System = '{source_name}'
                and src.Source_Object = '{source_object}'
                group by ls.Satellite_Identifier,ls.Target_Satellite_Table_Physical_Name,ls.Link_primary_key_physical_name,src.Source_Table_Physical_Name,src.Load_Date_Column
                """
