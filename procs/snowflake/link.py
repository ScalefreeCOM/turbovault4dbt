...
def generate_link_list(cursor, source):
    ...
    query = f"""SELECT l.Link_Identifier,Target_link_table_physical_name,LISTAGG(Target_column_physical_name) WITHIN GROUP (order by l.Target_Column_Sort_Order)
                from core.link_entities l
                inner join core.source_data src on src.Source_table_identifier = l.Source_Table_Identifier
                where 1=1
                and src.Source_System = '{source_name}'
                and src.Source_Object = '{source_object}'
                group by l.Link_Identifier,Target_link_table_physical_name
                """
...
def generate_source_models(cursor, link_id):
    ...
    query = f"""SELECT src.Source_Table_Physical_Name,LISTAGG(l.Hub_primary_key_physical_name) WITHIN GROUP  (ORDER BY Target_Column_Sort_Order),src.Static_Part_of_Record_Source_Column
                FROM core.link_entities l
                inner join core.source_data src on l.Source_Table_Identifier = src.Source_table_identifier
                where 1=1
                and Link_Identifier = '{link_id}'
                group by src.Source_Table_Physical_Name,src.Static_Part_of_Record_Source_Column
                """
...
def main(cursor, source, generated_timestamp, rdv_default_schema, model_path):
  ...
  for link in link_list:
    ...
    filename = os.path.join(model_path, generated_timestamp , f"{link_name.lower()}.sql")
