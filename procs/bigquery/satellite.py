...
def generate_satellite_list(bigquery_client, source):

    source_name, source_object = source.split("_")

    query = f"""SELECT DISTINCT hs.Satellite_Identifier,hs.Target_Satellite_Table_Physical_Name,hs.Hub_Primary_Key_Physical_Name,STRING_AGG(hs.Target_Column_Physical_Name order by Target_Column_Sort_Order asc),
                src.Source_Table_Physical_Name,src.Load_Date_Column
                from `dbt_automation_test.hub_satellites` hs
                inner join `dbt_automation_test.source_data` src on src.Source_table_identifier = hs.Source_Table_Identifier
                where 1=1
...
SELECT DISTINCT ls.Satellite_Identifier,ls.Target_Satellite_Table_Physical_Name,ls.Link_primary_key_physical_name,STRING_AGG(ls.Target_Column_Physical_Name order by Target_Column_Sort_Order asc),
                src.Source_Table_Physical_Name,src.Load_Date_Column
                from `dbt_automation_test.link_satellites` ls
                inner join `dbt_automation_test.source_data` src on src.Source_table_identifier = ls.Source_Table_Identifier
                where 1=1
                and src.Source_System = '{source_name}'
                and src.Source_Object = '{source_object}'
                group by 1,2,3,5,6
...
def main(bigquery_client,source, generated_timestamp, rdv_default_schema,model_path):
    ...
    satellite_list = generate_satellite_list(bigquery_client=bigquery_client, source=source)
    source_name, source_object = source.split("_")
